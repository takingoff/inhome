...
.roomStateIcon, .enteredStateIcon1, .enteredStateIcon2,
	.enteredStateIcon3 {
	float: right;
	padding-right: 2px;
}
			...
			success : function(data) {
				...
				for ( var index in data) {
					if (data[index].eVo != null) {
						...
						$("#trendContainerId").append(
								"<div class='room ${sessionScope.fore_Ground} ui-corner-all' id=\"roomId_" + data[index].rVo.id + "\" ondblclick='requestARoom(\""
										+ data[index].rVo.id + "\")'>" + "<div class='ui-widget-header '>" + data[index].rVo.room.name + "(" + data[index].rVo.room.roomType.name
										+ ")" + "</div>" + "<p >入住时间：" + data[index].eVo.enteredInfo.enteredTime + "<span class='roomStateIcon ui-icon '></span>" + "</p>"
										+ "<p >退房日期：" + data[index].eVo.enteredInfo.outTime + "<span class='enteredStateIcon1 '></span>" + "</p>" + "<p>联系人："
										+ data[index].eVo.enteredInfo.name + "(" + data[index].eVo.enteredInfo.numberPeople + "人)" + "<span class='enteredStateIcon2 '></span>"
										+ "</p>" + "<p>费用结算：" + billResult + "<span class='enteredStateIcon3 '></span>" + "</p>" + "<p>电话："
										+ data[index].eVo.enteredInfo.phoneNumber + "</p>" + "<p >说明：" + data[index].eVo.enteredInfo.description + "</p>" + "</div>");
						...
						$("#trendContainerId").append(
								"<div class='room ${sessionScope.fore_Ground} ui-corner-all' id=\"roomId_" + data[index].rVo.id + "\" ondblclick='requestARoom(\""
										+ data[index].rVo.id + "\")'>" + "<div class='ui-widget-header '>" + data[index].rVo.room.name + "(" + data[index].rVo.room.roomType.name
										+ ")" + "</div>" + "<p>状态：" + data[index].rVo.stateName + "<span class='roomStateIcon ui-icon '></span>" + "</p>" + "<p>说明："
										+ data[index].rVo.room.description + "</p>" + "</div>");
