/**
 * @fileOverview jui_datagrid is an Ajax-enabled jQuery plugin, useful to manipulate database data in tabular format.
 *               <p>License MIT
    ...
    var methods = {
        ...
        init: function(options) {
            ...
            return this.each(function() {
                ...
                elem.unbind("onDBClick").bind("onDBClick", settings.onDBClick);
...
//////////////////////////////////////////////////////////////////////////////////////////// modify by myself             
                //定义自己的filter类。将两个condition分别改为 group和condition
                function JuiElementFilter(group,condition,logicalOperator)
                { 
                    this.group=group;
                ...
                //filters 转换成 juiElementFilters(就是替换名字) 为递归调用
                function toJuiElementFilters(filters)
                {
                	 var juiElementFilters = new Array(filters.length);
                	 for(var i = 0; i < filters.length; i++)
                     {
                     	var juiElementFilter =new JuiElementFilter();
                     	juiElementFilter.logicalOperator = filters[i].logical_operator;
                     	//判定是否为 condition数组
                     	if(filters[i].condition[0] != undefined)
                     	{
                     		//替换掉 condition 换成 group
                     		juiElementFilter.group = toJuiElementFilters(filters[i].condition);
                     	}
                     	else
                     	{
                     		juiElementFilter.condition = filters[i].condition;
                     	}
                     	juiElementFilters[i] = juiElementFilter;
                     	
                     }
                	 return juiElementFilters;
                }
                var filters = elem.data(pluginStatus)['filter_rules'];
...
//                    	name:"John",name2:"Boston"
//                    },
                    
//                    data:'{"sortings":[{"sortingName":"id排序","field":"customer_id","order":"none"},{"sortingName":"姓排序","field":"lastname","order":"ASC"},{"sortingName":"名排序","field":"firstname","order":"ASC"}],"filters":[{"group":[{"element_rule_id":"rule_flt_rules_demo_grid1_2","condition":{"filterType":"text","field":"name","operator":"equal","filterValue":["asdf"]},"logical_operator":"AND"}],"logical_operator":"OR"},{"element_rule_id":"rule_flt_rules_demo_grid1_1","condition":{"filterType":"date","field":"genTime","operator":"less_or_equal","filterValue":["20140426232839"]},"logical_operator":"OR"}]}',
                    data:JSON.stringify(combineObj),
...
//                        }
                      ...
                      //  alert(data.pageData[0].roomType.name);
...
//                                        row_status;
                                	...
                                	var row_id = $(this).attr("id").substr(row_prefix_len),row_status;
...
////////////////////////////////////////////////////////////////////////////////////////////dbclick envent 
                            elem_table.off('dblclick', selector).on('dblclick', selector, function() {
                            	if(row_primary_key) {
//                                    var row_id = parseInt($(this).attr("id").substr(row_prefix_len)),
//                                        row_status;
                            	...
                            	elem.triggerHandler("onDBClick", {row_id: row_id});
                            });
// //////////////////////////////////////////////////////////////////////////////////////////dbclick
                        }
                        ...
                        if(settings.showPrefButton) {
                            ...
                            // tools tab
                            a_id_ext = ['_btn_select', '_btn_refresh', '_btn_delete', '_btn_print','_btn_add', '_btn_export', '_btn_sorting', '_btn_filters'];
                            a_opt = ['showSelectButtons', 'showRefreshButton', 'showDeleteButton', 'showPrintButton','showAddButton', 'showExportButton', 'showSortingButton', 'showFiltersButton'];
                            for(i in a_id_ext) {
                                util_pref(elem, elem_pref_dialog, "#" + pref_dialog_id + a_id_ext[i], a_opt[i]);
                            }
                                    ...
                                    switch(do_select) {
                                        ...
                                        case SELECT.inv_in_page:
                                            elem_row.each(function() {
                                                // get row id
//                                                var row_id = parseInt($(this).attr("id").substr(row_prefix_len));
                                                var row_id = $(this).attr("id").substr(row_prefix_len);
                                                var idx = $.inArray(row_id, elem.data(pluginStatus)['a_selected_ids']);
    ...
    var get_sorting_indicator = function(sorting, field) {
        ...
        for(var i in sorting) {
            if(sorting[i].field == field) {
                if(sorting[i].order == 'ASC') {
                    sorting_indicator = '&nbsp;&uarr;';
                } else if(sorting[i].order == 'DESC') {
    ...
    var create_preferences = function(plugin_container_id) {
        ...
        pref_html += util_pref_li(dialog_id + '_btn_add', rsc_jui_dg.pref_show_add);
    ...
    var create_sorting = function(plugin_container_id) {
        ...
        for(i in sorting) {
            ...
            sort_asc_radio_checked = sorting[i]["order"] == 'ASC' ? ' checked="checked"' : '';
            sort_desc_radio_checked = sorting[i]["order"] == 'DESC' ? ' checked="checked"' : '';
...
///////////////////////////////////////////////////////////////////////////////////modify by tang li // TODO

            var myFindIndex,myBeginIndex,myField,myFieldNameArray;
            var getValueFromField = function(obj,valueNames)
            {
            	var currentObj = obj;
            	for(var j = 0;j<valueNames.length;j++){
            		if(currentObj == null)
            			return "";
            		currentObj = currentObj[valueNames[j]];
            	}
            	return currentObj;
            };
            ...
            for(i in columns) {
                if(columns[i].visible == 'yes') {
                	...
                	myField = columns[i].field;
                	myFindIndex = 0;
                	myBeginIndex = 0;
                	myFieldNameArray = new Array();
                	while((myFindIndex =myField.indexOf(".",myBeginIndex+1)) != -1){
                		
                		myFieldNameArray.push(myField.substring(myBeginIndex,myFindIndex)) ;
                		myBeginIndex = myFindIndex +1;
                	}
                	myFieldNameArray.push(myField.substring(myBeginIndex));
                	
//                    tbl_html += '<td>' + page_data[row][columns[i].field] + '</td>';
                	tbl_html += '<td>' + getValueFromField(page_data[row],myFieldNameArray)+ '</td>';
                }
            }
...
//                    var row_id = parseInt($(this).attr("id").substr(row_prefix_len));
    ...
    var display_tools = function(container_id, total_rows, row_primary_key) {
        ...
        var elem = $("#" + container_id),
            ...
            showAddButton = elem.jui_datagrid('getOption', 'showAddButton'),
            showAddButtonText = elem.jui_datagrid('getOption', 'showAddButtonText'),
            tbAddIconClass = elem.jui_datagrid('getOption', 'tbAddIconClass'),
        ...
        if(showPrefButton&&total_rows>0) {
        ...
        if(total_rows >= 0) {
            if(showPrintButton || showExportButton || showAddButton) {
                tools_html += '<div class="' + tbButtonContainer + '">';
            }

            if(showAddButton) {
            	var add_id = tools_id + '_' + 'add';
            	tools_html += '<button id="' + add_id + '">' + rsc_jui_dg.tb_add + '</button>';
            }
        ...
        if(total_rows >= 0) {
            ...
            if(showAddButton) {
            	util_tools_btn($("#" + add_id), rsc_jui_dg.tb_add, showAddButtonText, tbAddIconClass);
            }
